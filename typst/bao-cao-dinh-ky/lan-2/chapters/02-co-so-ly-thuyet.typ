...
=== Sự cô lập dữ liệu và tối ưu hóa lưu trữ theo đặc thù nghiệp vụ

Việc áp dụng triệt để Database-per-service mang lại lợi thế chiến lược trong việc tối ưu hóa công nghệ lưu trữ cho từng vi dịch vụ. Hệ thống không bị ép buộc phải sử dụng một giải pháp cơ sở dữ liệu duy nhất cho mọi bài toán mà có thể tự do lựa chọn cấu hình lưu trữ tối ưu nhất cho đặc thù của từng vi dịch vụ, dựa trên nền tảng chung là PostgreSQL:

- Đối với dịch vụ Quản lý Đơn hàng (Order Service) và Quản lý Tài khoản (Account Service) - nơi yêu cầu tính nhất quán ACID tuyệt đối cho dòng tiền và trạng thái giao dịch, hệ quản trị cơ sở dữ liệu quan hệ PostgreSQL thuần được sử dụng.
- Đối với dịch vụ Tìm kiếm và Danh mục (Catalog Service) - nơi đặc thù nghiệp vụ là truy vấn thông tin đa chiều và tìm kiếm tương đồng vector, hệ thống tích hợp phần mở rộng pgvector trên nền PostgreSQL để lưu trữ và tính toán khoảng cách nhúng ngữ nghĩa tốc độ cao.

Mỗi vi dịch vụ nhờ đó chỉ cần vận hành đúng loại cấu hình dữ liệu phù hợp với đặc thù nghiệp vụ của mình, thay vì bị ràng buộc bởi một schema chung, cồng kềnh cho toàn hệ thống.
...
Nếu sử dụng mẫu thiết kế *Saga (Choreography hoặc Orchestration)* truyền thống ta phải đối mặt với một gánh nặng phát triển cực lớn:
...
- *Tự động phục hồi sau lỗi (Crash Recovery & Replay):* Khi một tiến trình dịch vụ đang xử lý giao dịch bị ngắt đột ngột (do crash ứng dụng, mất mạng hoặc nâng cấp máy chủ), Restate sẽ lập tức phát hiện và tự động điều phối khởi chạy lại hàm nghiệp vụ đó trên một node máy chủ khỏe mạnh khác. Quá trình phục hồi diễn ra qua cơ chế *Replay (Phát lại nhật ký)*: Restate duyệt qua đoạn code từ đầu, nhưng đối với các bước I/O đã có kết quả trong nhật ký, Restate lập tức trả về kết quả cũ mà *không hề kích hoạt lại lời gọi thực tế ra bên ngoài*. Luồng code tiếp tục chạy mượt mà từ chính điểm dừng gần nhất, loại bỏ hoàn toàn việc phải lập trình hàm bù trừ (Compensation) thủ công.
...
- *Quản lý trạng thái và định thời tích hợp (State & Timers):* Restate cung cấp các hàm đặt hẹn giờ bền vững (`ctx.sleep`) và lưu trữ trạng thái key-value nội bộ (`ctx.get/set`). Khi luồng Escrow cần chờ 72 giờ, hàm nghiệp vụ chỉ cần gọi `sleep(72 hours)`; Restate sẽ giải phóng toàn bộ tài nguyên RAM/CPU của tiến trình đó và tự động đánh thức (resume) luồng code chính xác sau 3 ngày mà không cần thiết lập thêm bất kỳ hệ thống cronjob hay hàng đợi phức tạp nào bên ngoài.
...
Để tận dụng tối đa năng lực phục hồi lỗi của Restate mà không làm hy sinh hiệu năng của các tác vụ truy vấn dữ liệu thông thường, hệ thống phân luồng giao tiếp API thành hai luồng riêng biệt:
...
- *Luồng Lệnh / Thao tác Ghi (Mutations - M) qua Restate Ingress Proxy:*
  ...
  - *Cơ chế hoạt động:* Restate Ingress đóng vai trò là một API Gateway. Khi tiếp nhận request, Ingress thực hiện kiểm duyệt bảo mật lớp đầu tiên (xác thực token JWT, kiểm tra quyền RBAC claims), gắn khóa `idempotency-key` từ header để chống lặp giao dịch, sau đó ghi nhận yêu cầu vào nhật ký bền vững (Write-Ahead Log) rồi mới điều hướng lời gọi RPC vào hàm bền vững (Durable Function) của microservice tương ứng.
...
- *Cơ chế Tín hiệu Đánh thức (Awakables / Promise Signaling):* Trong quy trình Escrow hoặc Dispute, quy trình nghiệp vụ thường phải tạm dừng để chờ một sự kiện xác nhận từ hệ thống bên ngoài (ví dụ: chờ webhook từ cổng thanh toán xác nhận đã thanh toán, hoặc chờ webhook từ đối tác vận chuyển GHN báo cáo đã giao hàng thành công). Restate cung cấp nguyên hàm `Awakable` - một đối tượng tương đương với Promise phân tán có định danh duy nhất. Luồng code gọi `ctx.awakable()` để tạo định danh và tạm dừng thực thi; khi hệ thống ngoại vi gửi webhook về Ingress kèm định danh này, Ingress lập tức giải phóng Awakable, "đánh thức" (resume) chính xác luồng Escrow đang ngủ và tiếp tục xử lý.
...
- *Dịch vụ lõi bền vững với Go (Golang):* Ngôn ngữ Go được lựa chọn làm công nghệ lập trình cho các microservices cốt lõi chịu trách nhiệm về giao dịch tài chính và quy trình dài hạn, bao gồm Order Service (Escrow/Dispute) và Account Service (Internal Wallet). Quyết định này xuất phát từ sự tương thích hoàn hảo giữa Go và nền tảng *Restate*. SDK Go của Restate là bộ thư viện đạt độ tối ưu cao nhất. Bên cạnh đó, Go biên dịch ra tệp thực thi nhị phân tĩnh (static binary) nhỏ, tiêu thụ tài nguyên bộ nhớ thấp, tốc độ khởi động ban đầu (cold start) dưới vài mili giây và mô hình xử lý đồng thời (goroutines) siêu nhẹ. Chấp nhận việc không sử dụng các ORM cồng kềnh mà viết code truy xuất dữ liệu trực tiếp bằng bộ công cụ *SQLC + pgx* (sinh code Go type-safe từ SQL chuẩn) nhằm đạt hiệu năng tối đa và tích hợp Restate mượt mà nhất.
