...
#show: a4.with(
  ...
  phu-de: "TUẦN 5, TUẦN 6 & TUẦN 7",
  chay: "Báo cáo tiến độ Tuần 5, 6 & 7",
)

= BÁO CÁO TIẾN ĐỘ TUẦN 5: THIẾT KẾ CẤP CAO

Tuần 5 chuyển từ kiến trúc tổng thể (Tuần 4) sang thiết kế cấp cao: chia hệ thống thành các thành phần có trách nhiệm rõ ràng, đặc tả giao diện API giữa frontend và backend, thiết kế cơ sở dữ liệu vật lý và các biện pháp bảo mật. Toàn bộ thiết kế bám theo kiến trúc Durable Microservices trên Restate và mô hình Database-per-service đã chốt ở Tuần 4.

== Thiết Kế Thành Phần (DOC 5.1-A)
...
#table(
  columns: (1fr, 0.7fr, 1.5fr, 0.6fr, 1.1fr),
...
Tuần 6 chi tiết hóa thiết kế cấp cao thành bản vẽ mà lập trình viên trực tiếp làm theo: cấu trúc lớp (thuộc tính, phương thức), sơ đồ trình tự ở mức lời gọi phương thức, thiết kế UI độ trung thực cao và tập lệnh DDL hoàn chỉnh.

== Thiết Kế Lớp (DOC 6.1-A)
...
#pagebreak()
== Sơ Đồ Trình Tự (DOC 6.2-A)

Ở Tuần 4 đã có sơ đồ trình tự mức nghiệp vụ durable (Escrow, Refund). Phần này bổ sung ba sơ đồ ở mức *lời gọi phương thức* giữa các lớp (Handler → Service → Repository → DB), xác thực rằng thiết kế lớp 6.1 thực sự chạy được, gồm một luồng Create, một Query và một Update có xử lý lỗi.
...
#pagebreak()
== Thiết Kế UI Độ Trung Thực Cao (DOC 6.3-A)

=== Hệ thống thiết kế
Kế thừa bố cục từ wireframe Tuần 3, bổ sung màu sắc, typography và thang khoảng cách cụ thể để hướng dẫn triển khai frontend.
...
Ba màn hình quan trọng nhất (Chi tiết sản phẩm C2C, Khung Chat & Offer Card, Thanh toán Escrow) được thiết kế độ trung thực cao dựa trên wireframe Tuần 3, áp dụng palette và typography ở trên; mỗi màn hình kèm chú thích khoảng cách (8px scale) và ghi chú cho lập trình viên. Bản mockup chi tiết được trình bày ngay dưới đây.
...
Ngoài ba màn hình cốt lõi, nhóm hoàn thiện mockup cho các màn hình còn lại của hành trình người dùng (trang chủ, tìm kiếm, đăng bán, quản lý bán hàng...). Toàn bộ bộ mockup được lưu trong thư mục `common/assets/mockups/`.
...
Tuần 7 khép lại giai đoạn thiết kế: đặc tả các thuật toán nghiệp vụ phức tạp, chiến lược xử lý lỗi, kế hoạch kiểm thử, chuẩn phát triển và lịch triển khai chi tiết cho giai đoạn code.

== Thiết Kế Thuật Toán (DOC 7.1-A)
...
- *Bên thứ ba:* mock server SePay/Stripe và GHN/GHTK để kiểm thử không phụ thuộc SLA ngoài (xem R-02, Tuần 4).
...
== Tiêu Chuẩn Phát Triển (DOC 7.4-A)
...
#pagebreak()
== Lập Kế Hoạch Triển Khai (DOC 7.5-A)
...
Kết thúc Tuần 7, toàn bộ tài liệu thiết kế cấp cao (thành phần, API, CSDL vật lý, bảo mật) và cấp thấp (lớp, trình tự, UI, DDL) cùng kế hoạch triển khai đã hoàn tất, sẵn sàng cho giai đoạn hiện thực hóa hệ thống ShopNexus C2C.
