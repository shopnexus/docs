...
Báo cáo thực tập tốt nghiệp đại học được bố cục thành 6 chương nội dung, thể hiện trọn vẹn tiến trình từ khảo sát bài toán, xây dựng nền tảng lý thuyết, phân tích yêu cầu, thiết kế hệ thống, cho đến hiện thực hóa và kiểm thử sản phẩm:
...
- *Chương 2: Cơ sở lý thuyết:* Trang bị nền tảng lý luận chuyên sâu làm chỗ dựa kỹ thuật cho toàn bộ dự án, bao gồm: Nguyên lý Kiến trúc Hướng dịch vụ (SOA), mô hình Microservices và triết lý Database-per-service; Lý thuyết về Thực thi bền vững (Durable Execution) và cơ chế Journal-based của nền tảng Restate; Các mô hình giao tiếp liên dịch vụ (lời gọi trực tiếp qua giao diện dịch vụ và Event-Driven trên Redis Streams / NATS JetStream); cùng Lý thuyết về nhúng ngữ nghĩa vector và cơ sở dữ liệu vector (pgvector, bge-m3).
- *Chương 3: Phân tích yêu cầu:* Xác định tầm nhìn và phạm vi, phân tích chân dung người dùng và các bên liên quan, sơ đồ ngữ cảnh hệ thống; đặc tả danh mục ca sử dụng và bộ quy tắc nghiệp vụ; mô hình hóa quy trình bằng sơ đồ hoạt động và sơ đồ trạng thái; mô hình dữ liệu ý niệm; bộ yêu cầu chức năng và phi chức năng định lượng kèm ma trận ưu tiên và ma trận truy xuất nguồn gốc.
- *Chương 4: Thiết kế hệ thống:* Trình bày các yếu tố dẫn dắt kiến trúc và lựa chọn công nghệ; kiến trúc tổng thể kèm các bản ghi quyết định kiến trúc (ADRs); sơ đồ thành phần và ma trận trách nhiệm; thiết kế API; thiết kế cơ sở dữ liệu vật lý kèm từ điển dữ liệu và mã DDL; thiết kế bảo mật; sơ đồ lớp chi tiết và sơ đồ trình tự; đặc tả thuật toán nghiệp vụ và chiến lược xử lý lỗi.
- *Chương 5: Hiện thực và triển khai:* Mô tả môi trường và công nghệ hiện thực, tiêu chuẩn phát triển và cấu trúc mã nguồn của ba thành phần (dịch vụ nền, ứng dụng web, ứng dụng di động); kết quả hiện thực từng module nghiệp vụ; tích hợp các nhà cung cấp bên thứ ba; quy trình CI/CD và mô hình triển khai bằng container.
- *Chương 6: Kiểm thử và đánh giá:* Trình bày chiến lược kiểm thử theo các mức, đặc tả và truy xuất ca kiểm thử, kết quả kiểm thử tự động, kết quả đo hiệu năng thực nghiệm, và đánh giá mức độ đáp ứng các yêu cầu phi chức năng đã đặt ra ở Chương 3.
...
Phạm vi nghiên cứu được xây dựng dựa trên sự kết hợp giữa các giải pháp nghiệp vụ (ví ký quỹ tạm giữ trong 72 giờ, thẻ đề nghị giao dịch Offer Card, cơ chế phân xử khiếu nại dựa trên bằng chứng số) và các phương pháp kỹ thuật được lựa chọn. Đây là nền tảng để triển khai các nội dung tiếp theo: hệ thống hóa cơ sở lý thuyết liên quan ở Chương 2, phân tích yêu cầu ở Chương 3, thiết kế hệ thống ở Chương 4, và hiện thực hóa cùng kiểm thử ở Chương 5 và Chương 6.