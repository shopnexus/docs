...
Nguyên lý cốt lõi của SOA là tính độc lập và ghép nối lỏng giữa các thành phần (Loose Coupling): các dịch vụ hạn chế tối đa sự phụ thuộc trực tiếp vào nhau, nên khi logic xử lý nội bộ hoặc cấu trúc dữ liệu riêng của một dịch vụ thay đổi thì các dịch vụ tiêu thụ (consumer services) không bị ảnh hưởng, miễn là hợp đồng API công khai (REST/gRPC/OpenAPI) vẫn được giữ nguyên [10].
...
Nếu sử dụng mẫu thiết kế Saga (Choreography hoặc Orchestration) truyền thống, các kỹ sư phần mềm phải đối mặt với một gánh nặng phát triển cực lớn. Trước hết là việc lập trình thủ công các hàm bù trừ: phải tự viết mã cho từng hàm thực thi xuôi (Action) kèm hàm bù trừ ngược (Compensation Action) tương ứng cho từng module, để hoàn tác dữ liệu khi có lỗi xảy ra giữa chừng [6]. Kế đó là xử lý rủi ro của hệ thống phân tán: khi gặp sự cố hạ tầng như mất kết nối mạng chập chờn, lỗi timeout, server bị khởi động lại hoặc tin nhắn hàng đợi bị gửi lặp lại (duplicate delivery), logic xử lý lỗi trong Saga thường phình to gấp nhiều lần mã nghiệp vụ chính, dễ gây sai lệch dòng tiền hoặc lặp giao dịch [8].

=== Khái niệm Durable execution và triết lý Journal-based Execution
Để khắc phục triệt để các nhược điểm của Saga mà không làm tăng độ phức tạp của mã nguồn nghiệp vụ, dự án lựa chọn mô hình durable execution. Ngữ nghĩa của mô hình này đã được đặc tả hình thức trong công trình về Durable Functions, nơi các tác giả chứng minh rằng một hàm nghiệp vụ được ghi nhật ký từng bước có thể chạy lại sau sự cố mà vẫn cho đúng kết quả như lần chạy đầu [4]. Đây là mô hình lập trình trong đó nền tảng thực thi đảm bảo một hàm nghiệp vụ có thể chạy liên tục đến khi hoàn thành, tự động tạm dừng khi chờ sự kiện ngoại vi và tự động phục hồi về đúng trạng thái trước khi lỗi xảy ra nếu máy chủ bị sập.
...
Để khắc phục rào cản trên, hệ thống ứng dụng công nghệ Tìm kiếm Ngữ nghĩa (Semantic Search) dựa trên biểu diễn không gian embedding. Khi một sản phẩm được tạo mới hoặc cập nhật tại `Catalog Service`, toàn bộ tiêu đề, mô tả và thuộc tính mặt hàng được truyền qua mô hình học sâu bge-m3 (BAAI General Embedding M3) để tạo ra các vector đặc trưng [5].

Mô hình bge-m3 được lựa chọn nhờ năng lực kiến trúc vượt trội, hỗ trợ tạo ra biểu diễn lai (Hybrid Representation) đồng thời trong một lần suy luận. Embedding dày đặc (Dense Embedding) là một vector 1024 chiều ($v in R^1024$). Vector này đóng gói toàn bộ ngữ nghĩa sâu xa và ý định của văn bản, đưa các khái niệm có ý nghĩa tương đồng lại gần nhau trong không gian vector. Nhờ đó, truy vấn "áo giữ ấm mùa đông" có thể tính toán khoảng cách rất gần với sản phẩm "áo khoác len cổ lọ dày dặn". Embedding thưa (Sparse Embedding hay Lexical Weights) sinh ra trọng số cho các từ vựng xuất hiện trong câu tương tự thuật toán BM25 [5]. Biểu diễn thưa này rất quan trọng trong thương mại điện tử vì nó giữ lại độ chính xác tuyệt đối cho các thông số kỹ thuật, mã model sản phẩm (như "RTX 4090", "M3 Max") - những từ khóa chuyên ngành rất dễ bị "hòa tan" và mất độ nét nếu chỉ dùng vector dày đặc.

=== Cơ sở dữ liệu vector pgvector và thuật toán kết hợp lai (Hybrid Search)
Thay vì phải vận hành thêm một hệ quản trị cơ sở dữ liệu vector độc lập (như Milvus, Pinecone hay Qdrant), điều sẽ làm tăng chi phí hạ tầng và phát sinh độ phức tạp lớn trong việc duy trì các pipeline bộ dữ liệu ETL từ cơ sở dữ liệu chính, ta sử dụng trực tiếp phần mở rộng mã nguồn mở pgvector được cài đặt ngay bên trong PostgreSQL của `Catalog Service` [12].
