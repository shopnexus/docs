...
Nguyên lý cốt lõi của SOA là tính độc lập và ghép nối lỏng giữa các thành phần (Loose Coupling): các dịch vụ hạn chế tối đa sự phụ thuộc trực tiếp vào nhau, nên khi logic xử lý nội bộ hoặc cấu trúc dữ liệu riêng của một dịch vụ thay đổi thì các dịch vụ tiêu thụ (consumer services) không bị ảnh hưởng, miễn là hợp đồng API công khai (REST/gRPC/OpenAPI) vẫn được giữ nguyên [9].
...
Để khắc phục rào cản trên, hệ thống ứng dụng công nghệ Tìm kiếm Ngữ nghĩa (Semantic Search) dựa trên biểu diễn không gian embedding. Khi một sản phẩm được tạo mới hoặc cập nhật tại `Catalog Service`, toàn bộ tiêu đề, mô tả và thuộc tính mặt hàng được truyền qua mô hình học sâu bge-m3 (BAAI General Embedding M3) để tạo ra các vector đặc trưng [4].

Mô hình bge-m3 được lựa chọn nhờ năng lực kiến trúc vượt trội, hỗ trợ tạo ra biểu diễn lai (Hybrid Representation) đồng thời trong một lần suy luận. Embedding dày đặc (Dense Embedding) là một vector 1024 chiều ($v in R^1024$). Vector này đóng gói toàn bộ ngữ nghĩa sâu xa và ý định của văn bản, đưa các khái niệm có ý nghĩa tương đồng lại gần nhau trong không gian vector. Nhờ đó, truy vấn "áo giữ ấm mùa đông" có thể tính toán khoảng cách rất gần với sản phẩm "áo khoác len cổ lọ dày dặn". Embedding thưa (Sparse Embedding hay Lexical Weights) sinh ra trọng số cho các từ vựng xuất hiện trong câu tương tự thuật toán BM25 [4]. Biểu diễn thưa này rất quan trọng trong thương mại điện tử vì nó giữ lại độ chính xác tuyệt đối cho các thông số kỹ thuật, mã model sản phẩm (như "RTX 4090", "M3 Max") - những từ khóa chuyên ngành rất dễ bị "hòa tan" và mất độ nét nếu chỉ dùng vector dày đặc.
