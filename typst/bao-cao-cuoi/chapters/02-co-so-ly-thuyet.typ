...
- *Tính độc lập và ghép nối lỏng giữa các thành phần (Loose Coupling):* Các dịch vụ hạn chế tối đa sự phụ thuộc trực tiếp vào nhau. Khi logic xử lý nội bộ hoặc cấu trúc dữ liệu riêng của một dịch vụ thay đổi, các dịch vụ tiêu thụ (consumer services) không bị ảnh hưởng, miễn là hợp đồng API công khai (REST/gRPC/OpenAPI) vẫn được giữ nguyên @newman2021.
...
- *Cô lập tuyệt đối về dữ liệu:* Mỗi vi dịch vụ sở hữu và quản lý độc lập một schema hoặc một instance cơ sở dữ liệu riêng biệt @richardson2019.
...
- Đối với dịch vụ Tìm kiếm và Danh mục (`Catalog Service`) - nơi đặc thù nghiệp vụ là truy vấn thông tin đa chiều và tìm kiếm tương đồng vector, hệ thống tích hợp phần mở rộng `pgvector` để lưu trữ và tính toán khoảng cách nhúng ngữ nghĩa tốc độ cao @pgvector2024.
